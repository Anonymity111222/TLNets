...
# SVD block
class SVD_Block(nn.Module):
    def __init__(self, modes1,modes2, F_channels):
        super(SVD_Block, self).__init__()
...
class F_SVD_Net(nn.Module):
    def __init__(self, channels, pred_num, F_channels):
        ...
        self.svd_block_1 = SVD_Block(channels, channels, F_channels)
...
class FFT_Conv_Net(nn.Module):
    def __init__(self, channels, pred_num, F_channels):
        ...
        self.channels = channels
        self.block = FFT_Conv(channels, pred_num)
...
class Conv_SVD(nn.Module):
    def __init__(self, channels, pred_num, F_channels):
        ...
        self.W_SVD = SVD_Block(channels, channels, F_channels)
...
class Conv_SVD_Net(nn.Module):
    def __init__(self, channels, pred_num, F_channels):
        ...
        self.block = Conv_SVD(channels, pred_num, F_channels)
...
class FT_matrix(nn.Module):
    def __init__(self, channels, pred_num, F_channels):
        super(FT_matrix, self).__init__()
...
class FT_matrix_Net(nn.Module):
    def __init__(self, channels, pred_num, F_channels):
        super(FT_matrix_Net, self).__init__()

        
        self.svd_block_1 = FT_matrix(channels, pred_num, F_channels)
        self.W_FFT_out = FFT1D_block(channels, channels, pred_num)
    def forward(self, x):
        x_ = x[:, -1:, :].detach()
        x = x - x_
